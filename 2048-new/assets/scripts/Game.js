cc.Class({
    ...
    onLoad() {
        ...
        wx.request({
            method: "post",
            url: 'https://www.31un.com/api/public/?s=App.HeroGame_User.Config',
            data: { },
            header: {
            ...
            success: (res) => {
                if (res.data.ret == 200) {
                   console.log(res.data.data)
                    if(res.data.data.is_review){
                       //审核模式
                        this.gamePro.isReview = true
                    }
                } else{
                    //强制变为审核模式
                    this.gamePro.isReview = true
    ...
    showRank() {
        ...
        if(this.maxSocre !=0){
            ...
            wx.request({
                method: "post",
                url: 'https://www.31un.com/api/public/?s=App.HeroGame_User.Result',
                data: { openID: this.userInfo.openID, name: this.userInfo.nickName, head: this.userInfo.avatarUrl, score: this.maxSocre, achievement: 1, ending: 1 },
                header: {
                    'content-type': 'application/x-www-form-urlencoded'
                },
                success: (res) => {
                    if (res.data.ret == 200) {
                        console.log('成绩已提交至全球排名')
                    }
                }
            })
        }else{
    ...
    onShareBtn(url){ //分享按钮
        ...
        //主动拉起分享接口
        if(url){
            cc.loader.load(self.gamePro.end.url, (err, data)=> {
                wx.shareAppMessage({
                    title: self.gamePro.end.text + '结局达成，最终得分'+ self.gamePro.kill,
                    imageUrl: data.url,
                    success(res){
                        console.log("转发成功!!!")
                    },
      ...
      if(this.gamePro.isReview){
        ...
        if(this.isNull(user)){
          ...
          if(this.gamePro.kill > this.maxSocre)
            ...
            //     method: "post",
            //     url: 'https://www.31un.com/api/public/?s=App.HeroGame_User.Result',
            //     data: { openID: this.userInfo.openID, name: this.userInfo.nickName, head: this.userInfo.avatarUrl, score: this.gamePro.kill, achievement: 1, ending: 1 },
            //     header: {
            //         'content-type': 'application/x-www-form-urlencoded'
            //     },
            //     success: (res) => {
    ...
    playCG(type, mode){
        ...
        if(type == 'save_princess_success'){
            ...
            this.gamePro.end = {
                text: '巨龙之怒',           // 结局
                url: 'https://album.31un.com/assets/cgs/share_cg4.jpg',            // cg图片
            }
